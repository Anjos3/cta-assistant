          ...
          <div className="grid md:grid-cols-2 gap-4">
            {topicLabs.map((lab, index) => (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="block p-4 border-2 border-gray-200 rounded-lg hover:border-secondary hover:shadow-md transition-all"
              >
                <div className="flex items-start gap-3">
                  <div className="w-12 h-12 bg-secondary rounded-lg flex items-center justify-center text-white flex-shrink-0 text-2xl">
                    🧪
                  </div>
                  <div className="flex-1">
                    <h4 className="font-bold text-gray-800 mb-1">{lab.name}</h4>
                    {lab.description && (
